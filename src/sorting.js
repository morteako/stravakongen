...
export const getSortingMode = arg => {
  if (arg.name)
    return () => (a, b) => a.athleteName.localeCompare(b.athleteName);
  ...
  if (arg.newest)
    return all => (a, b) => {
      const aMin = Object.values(all[a.athleteName]).map(
        x => new Date(x.start_date)
      );
      const bMin = Object.values(all[b.athleteName]).map(
        x => new Date(x.start_date)
      );
